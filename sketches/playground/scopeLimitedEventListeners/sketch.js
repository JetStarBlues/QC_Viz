...
/* TODO
   Limit scope of all canvas events to canvas.
   Use VanillaJS API, instead of p5 to attach event listeners.
*/
  // canvas.elt.addEventListener("mousemove", mouseMoved);
  // canvas.elt.addEventListener("mousedown", mousePressed);
  // canvas.elt.addEventListener("mouseup", mouseReleased);
  ...
  canvas.elt.addEventListener("touchcancel", touchEnded);
...
/* TODO
   Gradually replace all of these with their
   vanillaJS equivalent.

   Once done, test performance on multi-sketch page.
   Moving mouse around (especially outside a canvas),
   should no longer cause CPU spike.
*/
